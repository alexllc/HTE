...
split_half_testing <- function(covariates, Y,
    ...
    # if save_all is true, then observation level result will be stored.
    # Here file names is a file prefix for saving
    if((is_save | save_all) && is.null(file_prefix)) stop('if is_save or save_all is T, then file_prefix must be provided.')
    ...
    correlation_matrix <- foreach(i = seq(no_repeats), .combine = 'rbind', .options.multicore = list(set.seed = T)) %dopar%  {
        ...
        # }else if(sum(treatment == 0) > 0.05 * nrow(covariates)){
        #     treat <- seq(no.obs)[treatment != 0]
        #     control <- seq(no.obs)[treatment == 0]
        #     sampled_treat <- floor(length(treat)/2)
        #     trainId <- c(sample(treat, sampled_treat), sample(control, floor(no.obs/2) - sampled_treat))
        }else{
        ...
        tau.forest.train <- cf.estimator(covariates[trainId, ], Y[trainId], treatment[trainId], node.size = node.size, num_trees = num_trees, seed = seed)
        ...
        tau.forest.test <- cf.estimator(covariates[-trainId, ], Y[-trainId], treatment[-trainId], node.size = node.size, num_trees = num_trees, seed = seed)
        ...
        if(save_all){
            ...
            write.csv(observation_result.a, file = paste0(file_prefix, '.observation.', i, '.result.a.csv'))
            observation_result.b[-trainId,] <- tau.b.train.stats
            observation_result.b[trainId,] <- tau.b.stats
            write.csv(observation_result.b, file = paste0(file_prefix, '.observation.', i,'.result.b.csv'))
        }
        ...
        return(correlation_rslt)
    }

    if(is_save){
        colnames(correlation_matrix) <- col_names
        write.csv(correlation_matrix, file = paste0(file_prefix, '.split.half.csv'), row.names = F, quote = F)
    }
